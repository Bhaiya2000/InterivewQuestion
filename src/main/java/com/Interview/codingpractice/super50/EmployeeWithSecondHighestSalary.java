...
public class EmployeeWithSecondHighestSalary {
    public static void main(String[] args) {
        ...
        Employee secondHighestSalary = employeeList
                .stream().sorted(Comparator.comparingDouble(Employee::getSalary).reversed())
                .skip(1).findFirst().get();
        System.out.println(secondHighestSalary.getId()
                +" "+secondHighestSalary.getName()
                +" "+secondHighestSalary.getDepartment()
                +" "+secondHighestSalary.getSalary());
    }
}
